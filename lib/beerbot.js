...
beerbot.prototype._onMessage = function (message) {
    if (this._isChatMessage(message) &&
        ...
        this._isChannelConversation(message) &&
        !this._isFrombeerbot(message) &&
        this._isMentioninghveitibjormessage)
    ) {
...
beerbot.prototype._replyWithwhatever = function (originalMessage) {
    ...
    self.db.get('SELECT ID, beer, desc, einkunn, type FROM beer ORDER BY RANDOM() LIMIT 1', function (err, record){
        ...
        var channel = self._getChannelById(originalMessage.channel);
        self.postMessageToChannel(channel.name, 'beerbot mælir með ' + '*' + record.beer + '*' + ' sem er ' + record.desc + 'af tegundinni ' + '*' + record.type + '*' + ' og fær ' + record.einkunn + ' í einkunn á Untappd.', {as_user: true});
...
beerbot.prototype._replyWithhveitibjor = function (originalMessage) {
    var self = this;
    self.db.get('SELECT ID, beer, desc, einkunn FROM beer WHERE type="Hveitibjór" ORDER BY  RANDOM() LIMIT 1', function (err, record){
        if (err) {
            return console.error('DATABASE ERROR:', err);
        }
        var channel = self._getChannelById(originalMessage.channel);
